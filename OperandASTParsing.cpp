...
Imm16OperandExprAST* parseImm16Op(const std::string& operand) {
  ...
  long numVal = std::stol(operand.substr(1));
  if (numVal > std::numeric_limits<s16>::max() || numVal < std::numeric_limits<s16>::min()) {
    std::cout << "Operand value ("
              << numVal
              << ") exceeds 16-bit signed value; Taking lower 16-bits" << std::endl;
    numVal = numVal & ((1L << 16) - 1);
  }
...
Imm32OperandExprAST* parseImm32Op(const std::string& operand) {
  ...
  long numVal = std::stol(operand.substr(1));
  if (numVal > std::numeric_limits<s32>::max() || numVal < std::numeric_limits<s32>::min()) {
    std::cout << "Operand value ("
              << numVal
              << ") exceeds 32-bit signed value; Taking lower 32-bits" << std::endl;
    numVal = numVal & std::numeric_limits<u32>::max();
  }
