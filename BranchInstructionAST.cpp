...
bool BltInstructionAST::checkCondition(const Processor& processor) const {
  return processor.getStatusRegister().mN;
}
...
bool BleInstructionAST::checkCondition(const Processor& processor) const {
  return processor.getStatusRegister().mN || processor.getStatusRegister().mZ;
}
...
void CmpInstructionAST::eval(Processor& processor, u32 op2Value) const {
  ...
  const u32 u32Sum = op1 - op2Value;
  const u64 u64Sum = static_cast<u64>(op1) - static_cast<u64>(op2Value);

  const bool op1Neg = op1 & (1u << 31);
  const bool op2Neg = op2Value & (1u << 31);
  const bool sumNeg = u32Sum & (1u << 31);

  processor.getStatusRegister().mN = sumNeg;
  processor.getStatusRegister().mZ = u32Sum == 0;
  processor.getStatusRegister().mC = u64Sum & ~static_cast<u64>(std::numeric_limits<u32>::max());
