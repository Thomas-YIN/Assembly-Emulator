...
void RegMovInstructionAST::eval(Processor& processor) const {
  ...
  processor.getRegister(MovInstructionAST::getDst().getRegister()) = processor.getRegister(mSrc->getRegister());
}
...
void ImmMovInstructionAST::eval(Processor& processor) const {
  ...
  processor.getRegister(MovInstructionAST::getDst().getRegister()) = mImm->getSigned32();
}
...
void PushInstructionAST::eval(Processor& processor) const {
  ...
  for(s8 i=15; i>=0; i--){
    if(mRegList->hasRegister(i)){
      processor.getRegister(Register::SP) -= 4;
      processor.getStack().store(processor.getRegister(i), processor.getRegister(Register::SP));
    }
  }
}
...
void PopInstructionAST::eval(Processor& processor) const {
  ...
  for(u8 i=0; i<=15; i++){
    if(mRegList->hasRegister(i)){
      processor.getRegister(i) = processor.getStack().load(processor.getRegister(Register::SP));
      processor.getRegister(Register::SP) += 4;
    }
  }
}
...
void StrInstructionAST::eval(Processor& processor) const {
  ...
  processor.getStack().store(processor.getRegister(mSrc->getRegister()), processor.getRegister(mDst->getRegisterTarget().getRegister()) + mDst->getOffset());
}
...
void LdrInstructionAST::eval(Processor& processor) const {
  ...
  //       - You will need to apply the offset of the indirect operand to the memory address.
  //       - You do not need to shift the stack pointer.
  processor.getRegister(mDst->getRegister()) = processor.getStack().load(processor.getRegister(mSrc->getRegisterTarget().getRegister()) + mSrc->getOffset());
}
